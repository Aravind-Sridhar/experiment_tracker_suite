...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 25 09:02:45 2024

@author: jagdishkrishnaswamy
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sun Dec 22 08:36:59 2024
...
class CreateProjectDialog(QDialog):
    ...
    def save_project(self):
        """Save project metadata to the JSON file."""
        ...
        QMessageBox.information(self, "Project Saved", "Project created successfully.")
        self.accept()
...
class BrowseProjectsDialog(QDialog):
    ...
    def load_projects(self):
        """Load projects and their latest versions into the table."""
        self.projects_table.setRowCount(0)
        for row, project in enumerate(self.metadata.get("Projects", [])):
            project_name = project["name"]
            latest_version = str(max(map(int, project["versions"].keys())))
            version_summary = project["versions"][latest_version].get("version_summary", "No Summary")
            ...
            version_dropdown = QComboBox()
            version_dropdown.addItems(sorted(project["versions"].keys(), key=int, reverse=True))
            version_dropdown.setCurrentText(latest_version)
            version_dropdown.currentIndexChanged.connect(lambda _, r=row: self.version_selected(r))
            self.projects_table.setCellWidget(row, 1, version_dropdown)
