...
class ExperimentTracker(QMainWindow):
    ...
    def create_experiment_tab(self):
        ...
        # Add previous experiments selector
        self.previous_experiments_combo = QComboBox()
        self.previous_experiments_combo.currentIndexChanged.connect(self.load_experiment_template)
        ...
        self.version_lock = QPushButton("Lock Version for Editing")
        ...
        layout.addWidget(QLabel("Change Type:"))
        layout.addWidget(self.version_type_combo)
        layout.addWidget(self.version_lock)
            ...
            if fname.endswith('.json') and fname != project_file:
                try:
                    with open(os.path.join(self.base_folder, fname), 'r') as f:
                        data = json.load(f)
                        if 'experiment' in data and data['project_reference'] == project_file:
                            exp_title = data['experiment']['title']
                            version = data['experiment']['version']
                            display = f"{exp_title} (v{version})"
                            self.previous_experiments_combo.addItem(display, fname)
                except:
                    continue
    ...
    def load_selected_experiment(self):
        ...
        try:
            with open(os.path.join(self.base_folder, experiment_file), 'r') as f:
                ...
                self.version_type_combo.setCurrentText('Patch')  # Default to patch update
    ...
    def toggle_version_lock(self):
        if self.version_lock.isChecked():
            ...
            self.version_lock.setText("Version Editing Unlocked")
        else:
            self.version.setReadOnly(True)
            self.version_lock.setText("Lock Version for Editing")
            ...
            for f in os.listdir(self.base_folder):
                if f.endswith('.json'):
                    try:
                        ...
                        continue
        self.projects_combo.currentIndexChanged.connect(self.update_version)
